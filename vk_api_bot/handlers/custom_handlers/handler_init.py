...
def processing_init(user_state, user_id: int) -> None:
    """
    Функция запускает при состоянии init.
    Выводит картинку, текст приветствия в чат группы
    и меняет состояние пользователя на start
    """
    upload_image = upload.photo_messages(
        photos=str(BASE_DIR) + "/images/showcase.jpg")[0]
